...
# Model testing
def test_model(model_type: str = 'lstm'):
    """
    ...
    """
    print("\n" + SEP)
    print(f"TESTING {model_type.upper()} MODEL ARCHITECTURE")
    print(SEP + "\n")
    ...
    enc_input = torch.randint(0, tokenizer.vocab_size, (batch_size, src_len)).to(device)
    dec_input = torch.randint(0, tokenizer.vocab_size, (batch_size, tgt_len)).to(device)
    
    output = model(enc_input, dec_input)
    print(f"Forward pass successful!")
    print(f"Input shape: {enc_input.shape}")
    print(f"Output shape: {output.shape}")
    print(f"Expected shape: [batch={batch_size}, seq_len={tgt_len}, vocab_size={tokenizer.vocab_size}]")
    
    assert output.shape == (batch_size, tgt_len, tokenizer.vocab_size), "Output shape mismatch!"
...
def overfit_sanity_check(model_type: str = 'lstm', num_samples: int = 30, num_epochs: int = 150):
    ...
    if model_type == 'lstm':
        ...
        results_base = Path(__file__).parent / "results" / f"{model_type}_baseline"
    checkpoint_dir = results_base / "sanity_check"
...
def evaluate_model(model_type: str, study: str, dataset_split: str, checkpoint_path: str, device: str = "cpu") -> dict:
    ...
    # Load dataset
    data_path = Path(__file__).parent / "datasets" / study / f"{dataset_split}.json"
...
def main() -> None:
    ...
    parser.add_argument('--mode', type=str, default='train',
                choices=['generate', 'sanity', 'train', 'eval'],
                help='Mode to run')
